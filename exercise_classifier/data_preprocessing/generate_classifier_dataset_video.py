...
for video_path in files[FIRST_VIDEO:min(LAST_VIDEO, len(files))]:
    ...
    # TODO: Add the timeseries for angles
    counter = RepCounter(evolution)

    all_timeseries, all_extrema = counter.all_extrema()

    SEQUENCE_LENGTH = 50

    start_ind = 0

    print(len(list(all_timeseries["right_shoulder_angle"])))

    # print(all_timeseries.items())
    print(start_ind + SEQUENCE_LENGTH)

    print((start_ind + SEQUENCE_LENGTH) <= len(list(all_timeseries["right_shoulder_angle"])), (start_ind + SEQUENCE_LENGTH) ,"<=", len(list(all_timeseries["right_shoulder_angle"])))
    while (start_ind + SEQUENCE_LENGTH) <= len(list(all_timeseries["right_shoulder_angle"])):
        print("yooo")
        for name, timeseries in all_timeseries.items():
            print(name)
            if name not in dataset_dict:
                dataset_dict[name] = []
            print(len(list(timeseries)))
            sequence = list(zip(*list(timeseries)[start_ind:start_ind+SEQUENCE_LENGTH]))[1]

            dataset_dict[name].append(sequence)

        dataset_dict["exercise_label"] = exercise_label
        start_ind += 10
...
df.to_csv(F"video_dataset_{VIDEO_OFFSET}.csv")
